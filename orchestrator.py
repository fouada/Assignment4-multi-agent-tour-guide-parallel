...
class PointProcessor:
    ...
    def __init__(self, point: RoutePoint, result_callback: Callable[[JudgeDecision], None]):
        ...
        self.decision: Optional[JudgeDecision] = None
    ...
    def _run_agent(self, agent) -> Optional[ContentResult]:
...
class Orchestrator:
    ...
    def get_result(self, point_id: str) -> Optional[JudgeDecision]:
        """Get the result for a specific point."""
        with self.results_lock:
            return self.results.get(point_id)
    
    def get_next_result(self, timeout: float = None) -> Optional[JudgeDecision]:
        """
