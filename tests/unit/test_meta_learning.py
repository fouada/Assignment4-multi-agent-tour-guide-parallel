    ...
    def test_interaction_creation(self):
        ...
        interaction = UserInteraction(
            ...
            context_features=np.array(
                [0.5, 0.3, 0.2, 0.1, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9]
            ),
            rating=0.8,
...
class TestTask:
    ...
    def test_task_creation(self):
        ...
        support = [UserInteraction(ContentType.VIDEO, np.zeros(10), 0.5, 30.0)]
        query = [UserInteraction(ContentType.MUSIC, np.zeros(10), 0.7, 45.0)]

        task = Task(user_id="u1", support_set=support, query_set=query)
...
class TestMAML:
    ...
    def test_maml_creation(self):
        """Test MAML creation."""
        model = PreferenceModel(input_dim=10, hidden_dim=8, output_dim=3, seed=42)
        maml = MAML(model=model, inner_lr=0.01, outer_lr=0.001, inner_steps=5, seed=42)
    ...
    def test_outer_loop(self):
        ...
        for i in range(3):
            ...
            query = [UserInteraction(ContentType.MUSIC, np.random.rand(10), 0.6, 45.0)]
...
class TestReptile:
    ...
    def test_reptile_creation(self):
        ...
        model = PreferenceModel(input_dim=10, hidden_dim=8, output_dim=3, seed=42)
        reptile = Reptile(
            model=model, inner_lr=0.01, outer_lr=0.1, inner_steps=10, seed=42
        )
...
class TestColdStartHandler:
    ...
    def test_update_user_model(self):
        ...
        new_interaction = UserInteraction(
            ContentType.MUSIC, np.random.rand(10), 0.9, 60.0
        )
...
class TestPrototypicalNetworks:
    ...
    def test_creation(self):
        ...
        proto = PrototypicalNetworks(embedding_dim=16, n_clusters=5, seed=42)
...
class TestTaskGenerator:
    ...
    def test_generator_creation(self):
        ...
        generator = create_synthetic_task_generator(n_users=10, k_shot=5, seed=42)
