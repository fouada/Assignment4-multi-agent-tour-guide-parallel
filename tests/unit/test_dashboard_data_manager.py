...
class TestSimulationResult:
    ...
    def test_result_creation(self):
        ...
        result = SimulationResult(
            status=QueueStatus.COMPLETE, latency=5.0, num_results=3, quality=8.5
        )
    ...
    def test_failed_result(self):
        ...
        result = SimulationResult(
            status=QueueStatus.FAILED, latency=30.0, num_results=0, quality=0.0
        )
...
class TestSmartQueueSimulator:
    ...
    def test_edge_case_all_agents_succeed_fast(self):
        ...
        agents = [
            AgentConfig(
                "agent1", -2.0, 0.1, 0.01, 1.0, 9.0, 0.1
            ),  # Very fast, always succeeds
...
class TestStatisticalValidation:
    ...
    def test_mean_convergence(self):
        ...
        assert df_large["latency"].std() / np.sqrt(10000) < df_small[
            "latency"
        ].std() / np.sqrt(100)
