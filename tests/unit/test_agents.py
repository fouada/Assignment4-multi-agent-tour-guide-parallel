...
class TestVideoAgent:
    ...
    def test_mock_result_ammunition_hill(self):
        ...
        assert (
            "Ammunition" in result.title
            or "Battle" in result.title
...
class TestJudgeAgent:
    ...
    @patch(
        "src.agents.judge_agent.AGENT_SKILLS",
    ...
    def test_evaluate_two_candidates(
        ...
        with patch.object(
            agent,
            "_call_llm",
            return_value="WINNER: 1\nWINNER_SCORE: 8.5\nREASONING: Video is best",
        ):
            decision = agent.evaluate(
                mock_route_point, [mock_video_result, mock_music_result]
            )
...
REASONING: Text provides best educational value"""
    ...
    def test_quick_evaluate_location_boost(self, mock_route_point, mock_text_result):
        ...
        result = agent.quick_evaluate(
            mock_route_point, [video_result, mock_text_result]
        )
    ...
    def test_generate_single_candidate_reasoning(self, mock_text_result, adult_profile):
        ...
        reasoning = agent._generate_single_candidate_reasoning(
            mock_text_result, adult_profile
        )
...
REASONING: Text provides comprehensive historical context."""
        ...
        candidates = [
            ContentResult(
                point_id="p1", content_type=ContentType.VIDEO, title="V", source="Y"
            ),
            ContentResult(
                point_id="p1", content_type=ContentType.MUSIC, title="M", source="S"
            ),
            ContentResult(
                point_id="p1", content_type=ContentType.TEXT, title="T", source="W"
            ),
        ]
    ...
    def test_fallback_two_candidate_selection(
        self, mock_video_result, mock_music_result, adult_profile
    ):
        ...
        result = agent._fallback_two_candidate_selection(
            [mock_video_result, mock_music_result], type_preferences
        )
