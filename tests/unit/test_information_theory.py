...
class TestInformationTheoreticRegretBounds:
    ...
    def bounds(self):
        ...
        return InformationTheoreticRegretBounds(n_arms=3, arm_means=[0.8, 0.5, 0.6])
    ...
    def test_lai_robbins_bound(self, bounds):
        ...
        assert (
            "lower" in result.bound_type.lower() or "lai" in result.bound_type.lower()
        )
    ...
    def test_information_ratio_bound(self, bounds):
        ...
        assert result.bound_value > 0
        assert (
            "info" in result.bound_type.lower() or "ratio" in result.bound_type.lower()
        )
    ...
    def test_bounds_comparison(self, bounds):
        ...
        bounds.lai_robbins_bound(T).bound_value
...
class TestInformationTheoreticAnalyzer:
    ...
    def test_analyzer_with_custom_means(self):
        ...
        analyzer = InformationTheoreticAnalyzer(
            n_agents=4, arm_means=[0.8, 0.6, 0.5, 0.4]
        )
