...
class TestColorConfiguration:
    ...
    def test_colors_are_valid_hex(self):
        ...
        for _key, value in COLORS.items():
...
class TestSystemMonitorPanel:
    ...
    def test_create_throughput_chart_with_data(self):
        ...
        history = [
            {"timestamp": i, "throughput": np.random.uniform(5, 20)} for i in range(30)
        ]
...
class TestStatisticalComparisonPanel:
        ...
        df_a = pd.DataFrame(
            {
                ...
                "status": np.random.choice(
                    ["complete", "soft_degraded", "failed"], n, p=[0.7, 0.25, 0.05]
                ),
            }
        )
        ...
        df_b = pd.DataFrame(
            {
                ...
                "status": np.random.choice(
                    ["complete", "soft_degraded", "failed"], n, p=[0.6, 0.3, 0.1]
                ),
            }
        )
    ...
    def test_create_effect_size_chart(self, sample_comparison_stats):
        ...
        fig = StatisticalComparisonPanel.create_effect_size_chart(
            sample_comparison_stats
        )
    ...
    def test_create_significance_summary(self, sample_comparison_stats):
        ...
        fig = StatisticalComparisonPanel.create_significance_summary(
            sample_comparison_stats
        )
...
class TestMonteCarloPanel:
    ...
    def sample_mc_data(self):
        ...
        return pd.DataFrame(
            {
                "status": np.random.choice(
                    ["complete", "soft_degraded", "hard_degraded", "failed"],
                    n,
                    p=[0.6, 0.25, 0.1, 0.05],
                ),
    ...
    def test_create_convergence_plot(self):
        ...
        fig = MonteCarloPanel.create_convergence_plot(
            n_values, metric_values, "Mean Latency"
        )
...
class TestVisualizationPerformance:
    ...
    def test_large_dataset_handling(self):
        ...
        large_df = pd.DataFrame(
            {
                "status": np.random.choice(
                    ["complete", "soft_degraded", "failed"], 10000
                ),
