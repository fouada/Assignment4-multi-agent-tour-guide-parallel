...
class TestSHAPExplainer:
    ...
    def shap_explainer(self, simple_model):
        ...
        features = [
            Feature.CONTENT_RELEVANCE,
            Feature.CONTENT_QUALITY,
            Feature.USER_IS_DRIVER,
        ]
    ...
    def test_explainer_with_baseline(self, simple_model):
        ...
        explainer = SHAPExplainer(
            model=simple_model, features=features, baseline=baseline
        )
...
class TestLIMEExplainer:
    ...
    def test_lime_with_custom_kernel_width(self, simple_model):
        ...
        explainer = LIMEExplainer(
            model=simple_model, features=features, kernel_width=0.5
        )
...
class TestCounterfactualExplainer:
    ...
    def decision_model(self):
        ...
        def model(features: dict[Feature, float]) -> str:
            score = 0.4 * features.get(
                Feature.CONTENT_QUALITY, 0.5
            ) + 0.3 * features.get(Feature.CONTENT_RELEVANCE, 0.5)
            if score > 0.5:
                return "video"
            elif score > 0.3:
...
class TestExplainabilityEngine:
    ...
    def decision_model(self):
        ...
        def model(features: dict[Feature, float]) -> str:
            score = (
                features.get(Feature.CONTENT_RELEVANCE, 0.5) * 0.6
                + features.get(Feature.CONTENT_QUALITY, 0.5) * 0.4
            )
            if score > 0.6:
                return "video"
            elif score > 0.4:
