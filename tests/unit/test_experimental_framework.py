...
class TestExperimentConfig:
    ...
    def test_custom_values(self):
        ...
        config = ExperimentConfig(
            name="custom_exp", seed=123, n_replications=50, parameters=params
        )
    ...
    def test_to_dict(self):
        ...
        config = ExperimentConfig(name="test", seed=123, parameters={"x": 10})
...
class TestExperimentResult:
    ...
    def test_failed_result(self, sample_config):
        ...
        result = ExperimentResult(
            config=sample_config, status="failed", error_message="Test error"
        )
...
class TestExperimentRunner:
    ...
    def runner(self, tmp_path):
        return ExperimentRunner(
            experiment_class=DummyExperiment, output_dir=tmp_path, verbose=False
        )
    ...
    def test_run_parameter_sweep(self, runner):
        ...
        base_config = ExperimentConfig(
            name="sweep", n_replications=2, parameters={"base_param": 1}
        )

        results = runner.run_parameter_sweep(
            base_config, parameter_grid={"x": [1, 2], "y": [10, 20]}
        )
    ...
    def test_parameter_sweep_preserves_base(self, runner):
        ...
        base_config = ExperimentConfig(
            name="sweep", n_replications=2, parameters={"fixed": 100}
        )

        results = runner.run_parameter_sweep(
            base_config, parameter_grid={"varied": [1, 2]}
        )
    ...
    def test_compare_results(self, runner):
        ...
        for i in range(3):
            config = ExperimentConfig(
                name=f"comp_{i}", n_replications=5, parameters={"variant": i}
            )
            runner.run_single(config)
    ...
    def test_verbose_logging(self, tmp_path, capsys):
        ...
        runner = ExperimentRunner(
            experiment_class=DummyExperiment, output_dir=tmp_path, verbose=True
        )
...
class TestFactorialDesign:
    ...
    def test_full_factorial_2k_three_factors(self):
        ...
        factors = {"X": (10, 20), "Y": (100, 200), "Z": (0.1, 0.9)}
    ...
    def test_full_factorial_values(self):
        ...
        factors = {"temp": (20, 80), "pressure": (1, 10)}
    ...
    def test_add_center_points(self):
        ...
        factors = {"A": (0, 10), "B": (100, 200)}

        designs = FactorialDesign.full_factorial_2k(factors)
        designs_with_center = FactorialDesign.add_center_points(
            designs, factors, n_center=3
        )
...
class TestEdgeCases:
    ...
    def test_nested_parameters(self):
        ...
        config = ExperimentConfig(
            name="nested", parameters={"outer": {"inner": [1, 2, 3]}}
        )
    ...
    def test_runner_missing_metric(self, tmp_path):
        ...
        runner = ExperimentRunner(DummyExperiment, output_dir=tmp_path, verbose=False)
