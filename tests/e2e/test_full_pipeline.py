...
from src.models.user_profile import UserProfile, Gender, AgeGroup, ContentPreference
...
class E2ETestResult:
    ...
    point_id: str
    selected_content: Optional[ContentResult]
...
class TestFullTourPipeline:
    ...
    def test_full_tour_flow_multiple_points(
        ...
        for point in sample_route_points:
            ...
            # Judge selection
            judge = JudgeAgent()
            decision = judge.evaluate(content_results, family_profile, point)
        ...
        for point_id, result in results_by_point.items():
    ...
    def test_tour_flow_with_orchestrator(
        self, sample_route_points, family_profile
    ):
        """Test complete flow using the Orchestrator component."""
        with patch('src.core.orchestrator.Orchestrator._execute_agents') as mock_execute:
            # Setup mock to return content for each point
            ...
            # Process route
            tour_results = orchestrator.process_route(
                points=sample_route_points[:2],  # Test with 2 points
                user_profile=family_profile,
            )
            
            # Validate orchestrator output
            assert tour_results is not None
            assert len(tour_results.point_results) == 2
    ...
    def test_parallel_processing_performance(
        ...
        def simulate_agent_work(point: RoutePoint, content_type: ContentType) -> ContentResult:
