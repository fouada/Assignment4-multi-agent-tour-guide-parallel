...
class ISO25010ComplianceChecker:
            ...
            for pattern in secret_patterns:
                if re.search(pattern, content):
                    ...
                    if "example" in content.lower() or "placeholder" in content.lower():
                        continue
                    return CheckResult.FAIL, f"Potential hardcoded secret in {py_file.name}"
    ...
    def _check_yaml_config(self) -> tuple[CheckResult, str]:
        ...
        if config_dir.exists() or agent_configs.exists():
            ...
            yaml_files += list(agent_configs.glob("*.yaml")) if agent_configs.exists() else []
            if yaml_files:
    ...
    def print_report(self, verbose: bool = False) -> None:
        ...
        for char_name, char_result in self.results.items():
            ...
            icon = "✅" if level == ComplianceLevel.FULL else "⚠️" if level == ComplianceLevel.PARTIAL else "❌"
            ...
            if verbose:
                for sub_name, checks in char_result.sub_characteristics.items():
                    print(f"\n  📋 {sub_name}")
                    for check in checks:
                        result_icon = "✅" if check.result == CheckResult.PASS else "❌" if check.result == CheckResult.FAIL else "⚠️"
                        print(f"    {result_icon} {check.name}")
                        if check.result != CheckResult.PASS:
                            print(f"       → {check.message}")
        
        # Overall compliance
        all_full = all(r.compliance_level == ComplianceLevel.FULL for r in self.results.values())
    ...
    def to_json(self) -> str:
        ...
        for char_name, char_result in self.results.items():
            ...
            for sub_name, checks in char_result.sub_characteristics.items():
                report["characteristics"][char_name]["sub_characteristics"][sub_name] = [
                    {
                        "name": c.name,
