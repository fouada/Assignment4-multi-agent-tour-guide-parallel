...
class MutualInformationCalculator:
    ...
    def conditional(
        ...
        p_z = joint_xyz.sum(axis=(0, 1))  # P(Z)
...
class InformationTheoreticRegretBounds:
    ...
    def lai_robbins_bound(
        self,
        T: int,
        arm_variances: Optional[list[float]] = None
    ) -> RegretBoundResult:
        """
        ...
        return RegretBoundResult(
            ...
            parameters={
                ...
                ) if any(m < self.optimal_mean for m in self.arm_means) else 0,
            },
            interpretation=(
...
class AgentUserChannel:
    ...
    def compute_capacity(
        ...
        capacity = MutualInformationCalculator.compute(
            np.outer(q, np.ones(n_y)) * P
        )
...
class DiversityMetrics:
    ...
    def conditional_diversity(
        ...
        for context, counts in selections_by_context.items():
