...
class ResearchVisualizer:
    ...
    def compare_distributions(
        ...
        sns.histplot(
            ...
            alpha=0.5,
            label=labels[1],
            color=self.COMPARISON_COLORS[1],
            kde=True,
            ax=ax1,
        )
    ...
    def sensitivity_plot(
        ...
        error_bars: dict[str, np.ndarray] | None = None,
        save_name: str | None = None,
    ):
        """
    ...
    def pareto_frontier(
        ...
        if color_by is not None:
            scatter = ax.scatter(
                x[~is_pareto],
                y[~is_pareto],
                c=color_by[~is_pareto],
                cmap=self.GRADIENT_CMAP,
                alpha=0.4,
                s=60,
                edgecolors="gray",
            )
        ...
        if highlight_pareto and np.any(is_pareto):
            ...
            ax.plot(
                pareto_x[sort_idx], pareto_y[sort_idx], "r--", alpha=0.5, linewidth=2
            )
    ...
    def effect_size_forest_plot(
        ...
        for boundary, _label in [(0.2, "Small"), (0.5, "Medium"), (0.8, "Large")]:
...
def create_publication_figure(
    figsize: tuple[float, float] = (7, 5), n_rows: int = 1, n_cols: int = 1
):
    """
