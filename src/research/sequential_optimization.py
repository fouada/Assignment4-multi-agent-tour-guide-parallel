...
class TourState:
    ...
    def to_vector(self) -> np.ndarray:
        ...
        if len(self.content_history) >= 2:
            consecutive = 1 if self.content_history[-1] == self.content_history[-2] else 0
        else:
            consecutive = 0
...
class SequentialContentOptimizer:
    ...
    def _sample_location_type(self) -> str:
        ...
        types = ["historical", "scenic", "cultural", "entertainment", "religious", "natural"]
    ...
    def generate_optimal_sequence(
        ...
        location_significances: list[float]
    ...
    def theoretical_analysis(self) -> dict[str, Any]:
        ...
        return {
            ...
            "convergence_metrics": {
                "final_100_avg_reward": np.mean(self.episode_rewards[-100:]) if len(self.episode_rewards) >= 100 else np.mean(self.episode_rewards),
            ...
            "theoretical_guarantees": {
                ...
                "sample_complexity": f"O(1/ε²) episodes for ε-optimal policy",
            }
        }
...
class DiversityConstrainedOptimizer(SequentialContentOptimizer):
    ...
    def __init__(
        ...
        self.lambdas = {ct: 0.0 for ct in ContentType}
    ...
    def train_episode_constrained(self) -> dict[str, float]:
        ...
        if len(self.episode_rewards) >= 10:
            ...
            type_fractions = {ct: 1/3 for ct in ContentType}  # Placeholder
...
def demo_sequential_optimization():
    ...
    location_types = ["historical", "scenic", "cultural", "entertainment", 
                      "natural", "religious", "scenic", "historical"]
    ...
    for i, (loc, content, conf) in enumerate(sequence):
        ...
        print(f"   {i+1}. {loc.upper():12} → {emoji} {content.value.upper():6} (conf: {conf:.2f})")
    
    # Theoretical analysis
    print("\n📐 Theoretical Analysis:")
    analysis = optimizer.theoretical_analysis()
    print(f"   Final Avg Reward: {analysis['convergence_metrics']['final_100_avg_reward']:.3f}")
    print(f"   Policy Gradient Theorem: {analysis['theoretical_guarantees']['policy_gradient_theorem']}")
