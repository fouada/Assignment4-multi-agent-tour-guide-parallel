...
class SHAPExplainer:
    ...
    def __init__(
        ...
        baseline: Optional[dict[Feature, float]] = None
    ):
        """
        ...
        self.baseline = baseline or {f: 0.5 for f in features}
        ...
        for i, feature in enumerate(self.features):
            ...
            for s in range(n):
                ...
                for subset in combinations(range(n - 1), s):
                    ...
                    input_with_i[feature] = instance.get(feature, self.baseline[feature])
...
class CounterfactualExplainer:
            ...
            if cf_decision == target_decision:
                ...
                if changes:
                    cf = CounterfactualExplanation(
                        ...
                        changes=changes,
                        total_change=np.sum(np.abs(
                            best_x - np.array([instance.get(f, 0.5) for f in self.features])
                        )),
                        plausibility_score=self._compute_plausibility(instance, cf_dict),
                    )
    ...
    def _compute_plausibility(
        self,
        original: dict[Feature, float],
        counterfactual: dict[Feature, float]
    ) -> float:
        """
...
class ExplainabilityEngine:
    ...
    def __init__(
        ...
        self.counterfactual = CounterfactualExplainer(
            decision_model,
            lambda x, t: self._target_score(x, t),
            self.features
        )
    ...
    def explain_decision(
        ...
        if ExplanationType.FEATURE_IMPORTANCE in methods:
            ...
            explanations["shap_values"] = {
                f.value: v for f, v in shap_values.items()
            }
        ...
        if ExplanationType.COUNTERFACTUAL in methods:
            other_agents = [a for a in ["video", "music", "text"] 
                          if a != decision.selected_agent]
        ...
        if ExplanationType.NATURAL_LANGUAGE in methods:
            if "shap_values" in explanations:
                nl_explanation = self.nl_explainer.explain(
                    decision,
                    {Feature(k): v for k, v in explanations["shap_values"].items()
