...
class CausalObservation:
    ...
    def get(self, var: CausalVariable) -> Optional[float]:
...
class StructuralEquation:
    ...
    def compute(
        self,
        parent_values: dict[CausalVariable, float],
...
class StructuralCausalModel:
    ...
    def sample(
        self,
        exogenous_values: Optional[dict[CausalVariable, float]] = None,
        intervention: Optional[dict[CausalVariable, float]] = None
...
class CausalEffectEstimator:
    ...
    def _bootstrap_se(
        ...
        for _ in range(n_bootstrap):
            ...
            if np.sum(treated) > 0 and np.sum(control) > 0:
                y1 = np.mean(y_boot[treated] / p_boot[treated]) * np.mean(p_boot[treated])
                y0 = np.mean(y_boot[control] / (1 - p_boot[control])) * np.mean(1 - p_boot[control])
    ...
    def estimate_cate(
        ...
        for i, label in enumerate(bin_labels):
            ...
            ate, se = sub_estimator.estimate_ate_ipw(
        ...
        for depth in range(n_vars - 1):
            for i in range(n_vars):
                for j in range(i + 1, n_vars):
                    ...
                    for subset in itertools.combinations(neighbors, min(depth, len(neighbors))):
...
class AgentPerformanceAnalyzer:
    ...
    def _interpret_effect(
        ...
        if abs(z) < 1.96:
            return f"No statistically significant effect of selecting {agent_type} agent."
        elif ate > 0:
            return (
    ...
    def generate_report(self) -> dict[str, Any]:
        ...
        report["recommendations"].append({
            ...
            "confidence": "high" if effects[best_agent]["significant_at_05"] else "medium",
        })
...
def demo_causal_inference():
    ...
    print(f"   Difference: {cf_satisfaction - observation[CausalVariable.USER_SATISFACTION]:+.3f}")
