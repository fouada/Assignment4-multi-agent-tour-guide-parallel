    ...
    def __init__(
        ...
        for _i in range(n_layers):
            if use_attention:
                ...
                layer = GraphConvLayer(hidden_dim, hidden_dim, seed=seed)
...
class RouteAwareContentSelector:
    ...
    def build_graph(self, locations: list[dict[str, Any]]) -> RouteGraph:
        """
    ...
    def select_content(
        self, locations: list[dict[str, Any]], enforce_diversity: bool = True
    ...
    def _enforce_diversity(
        self, probs: np.ndarray, min_type_fraction: float = 0.2
    ) -> np.ndarray:
        """
    ...
    def _compute_diversity(
        self, results: list[tuple[str, ContentType, float]]
    ) -> float:
    ...
    def get_theoretical_analysis(self) -> dict[str, Any]:
        ...
        return {
            ...
            "theoretical_properties": {
                ...
                "attention_benefit": "Learns to weight important neighbors"
            ...
            "avg_diversity": np.mean(
                [h["diversity_score"] for h in self.prediction_history]
            )
...
def demo_graph_neural():
    ...
    selector = RouteAwareContentSelector(
        hidden_dim=64, n_layers=3, use_attention=True, seed=42
    )
    ...
    for name, content_type, confidence in results:
        ...
        print(
            f"   {name:20} → {emoji} {content_type.value.upper():6} (conf: {confidence:.3f})"
        )
