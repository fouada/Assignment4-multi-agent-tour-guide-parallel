...
class ExperimentRunner:
    ...
    def run_single(self, config: ExperimentConfig) -> ExperimentResult:
        ...
        # Save result
        result_path = self.output_dir / f"{config.name}_{config.metadata['config_hash']}.json"
...
class FactorialDesign:
    ...
    @staticmethod
    def full_factorial_2k(factors: Dict[str, Tuple[float, float]]) -> List[Dict[str, float]]:
        """
        ...
        for combo in product(*levels):
            designs.append(dict(zip(factor_names, combo)))
    ...
    def analyze_effects(
        ...
        for i, f1 in enumerate(factor_names):
            for f2 in factor_names[i+1:]:
                high_high = np.mean([r for p, r in coded_results if p[f1] > 0 and p[f2] > 0])
                low_low = np.mean([r for p, r in coded_results if p[f1] < 0 and p[f2] < 0])
                high_low = np.mean([r for p, r in coded_results if p[f1] > 0 and p[f2] < 0])
                low_high = np.mean([r for p, r in coded_results if p[f1] < 0 and p[f2] > 0])
                
                effects[f'{f1}:{f2}'] = ((high_high + low_low) - (high_low + low_high)) / 2
