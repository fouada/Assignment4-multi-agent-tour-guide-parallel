...
class PredictionSet:
    ...
    def point_prediction(self) -> Optional[ContentType]:
...
class SelectivePredictor:
    ...
    def predict_or_abstain(
        self,
        predictions: np.ndarray
    ) -> tuple[Optional[ContentType], str, dict]:
        """
...
class UncertaintyAwareContentSelector:
    ...
    def __init__(
        ...
        # Selective predictor
        if allow_abstention:
            base_predictor = ConformalPredictor(self.scorer, coverage=coverage, seed=seed)
    ...
    def calibrate(
        ...
        for i in range(len(true_labels)):
            self.predictor.predict_and_update(
                model_predictions[i],
                int(true_labels[i])
            )
    ...
    def select_content(
        ...
        if self.use_adaptive:
            pred_set = self.predictor.predict_and_update(
        ...
        if self.allow_abstention:
            ...
            if pred_set.size > 2 or max_prob < 0.3:
                abstained = True
                abstention_reason = "uncertain" if pred_set.size > 2 else "low_confidence"
    ...
    def get_theoretical_analysis(self) -> dict[str, Any]:
        ...
        if self.selection_history:
            ...
            singleton_rate = np.mean([s["set_size"] == 1 for s in self.selection_history])
...
def demo_uncertainty_quantification():
    ...
    cal_labels = np.array([
        rng.choice(3, p=p) for p in cal_predictions
    ])
    ...
    print(f"   Distribution-Free: {theory['theoretical_guarantees']['distribution_free']}")
