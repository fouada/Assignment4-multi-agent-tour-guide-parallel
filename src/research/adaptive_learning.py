...
class Reward:
    ...
    explicit_feedback: Optional[float] = None  # User rating if available
...
class BanditStatistics:
    ...
    arm_pulls: dict[AgentType, int] = field(
        default_factory=lambda: {a: 0 for a in AgentType}
    )
    arm_rewards: dict[AgentType, float] = field(
        default_factory=lambda: {a: 0.0 for a in AgentType}
    )
...
class MultiArmedBandit(ABC):
    ...
    def __init__(self, arms: list[AgentType], seed: Optional[int] = None):
        """
    ...
    @abstractmethod
    def select_arm(self, context: Optional[Context] = None) -> AgentType:
        """
...
class UCB(MultiArmedBandit):
    ...
    def __init__(
        ...
        self.arm_means: dict[AgentType, float] = {arm: 0.0 for arm in arms}
        self.arm_counts: dict[AgentType, int] = {arm: 0 for arm in arms}
        self.total_count = 0
...
class ContextualThompsonSampling(MultiArmedBandit):
    ...
    def __init__(
        ...
        self.mu: dict[AgentType, np.ndarray] = {
            arm: np.zeros(self.d) for arm in arms
        }
        ...
        self.Xty: dict[AgentType, np.ndarray] = {
            arm: np.zeros(self.d) for arm in arms
        }
...
class AdaptiveAgentSelector:
    ...
    def select_agent(self, context: Optional[Context] = None) -> AgentType:
        """
    ...
    def get_arm_probabilities(self, context: Optional[Context] = None) -> dict[AgentType, float]:
        """
        ...
        if isinstance(self.bandit, ThompsonSampling):
            ...
            means = {
                arm: self.bandit.get_posterior_mean(arm)
                for arm in AgentType
            }
            ...
            indices = {
                arm: self.bandit.get_ucb_index(arm)
                for arm in AgentType
            }
            # Softmax
            max_idx = max(indices.values())
            exp_vals = {
                arm: math.exp(idx - max_idx)
...
class BanditExperiment:
    ...
    def run_experiment(
        ...
        num_rounds: int = 1000,
        context_generator: Optional[Callable[[], Context]] = None
    ) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "arm_distribution": {
                arm.value: arm_selections.count(arm) / num_rounds
                for arm in AgentType
            }
        }
