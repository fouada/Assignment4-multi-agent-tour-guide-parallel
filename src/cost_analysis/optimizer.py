...
class OptimizationStrategy:
    ...
    def total_savings_potential(self) -> float:
        ...
        if self.total_current_cost == 0:
            return 0.0
        return (
            (self.total_current_cost - self.total_optimized_cost)
    ...
    def get_by_priority(
        self, priority: OptimizationPriority
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "by_priority": {
                p.value: len(self.get_by_priority(p)) for p in OptimizationPriority
            },
            "by_category": {
                c.value: len(self.get_by_category(c)) for c in OptimizationCategory
            },
        }
...
class CostOptimizer:
    ...
    def generate_strategy(
        ...
        recommendations.extend(
            self._analyze_caching_potential(monthly_cost, tour_summaries)
        )
        ...
        total_optimized = monthly_cost * (
            1 - sum(r.savings_potential for r in recommendations) / 100
        )
    ...
    def _analyze_model_selection(
        ...
        if "gpt-4o" in current_model and "mini" not in current_model:
            ...
            current_rate = (
                pricing.ANTHROPIC_CLAUDE_SONNET_INPUT
                + pricing.ANTHROPIC_CLAUDE_SONNET_OUTPUT
            )
        else:
            current_rate = (
    ...
    def _analyze_batching_potential(
        self, monthly_cost: float
    ) -> list[OptimizationRecommendation]:
                    ...
                    roadmap.append(
                        {
                            ...
                            "expected_savings": sum(
                                i["savings_potential"] for i in phase_items
                            ),
                        }
                    )
                phase += 1
...
class CostAwareConfigOptimizer:
    ...
    def optimize_for_budget(
        ...
        models = [
            ...
            },
            {
                ...
                "latency": 1.0,
                "cost_per_1k_tokens": (
                    pricing.ANTHROPIC_CLAUDE_HAIKU_INPUT
                    + pricing.ANTHROPIC_CLAUDE_HAIKU_OUTPUT
                )
                / 1000,
            },
            {
                ...
                "latency": 1.0,
                "cost_per_1k_tokens": (
                    pricing.OPENAI_GPT35_INPUT + pricing.OPENAI_GPT35_OUTPUT
                )
                / 1000,
            },
        ]
        ...
        for model in sorted(valid_models, key=lambda m: m["cost_per_1k_tokens"]):
            monthly_cost = (estimated_monthly_tokens / 1000) * model[
                "cost_per_1k_tokens"
            ]
            if monthly_cost <= monthly_budget_usd:
                return {
                    ...
                    "recommendations": self._get_config_recommendations(
    ...
    def _get_config_recommendations(
        ...
        recommendations.append(
            "Set max_tokens to 500 for content agents, 300 for judge"
        )
