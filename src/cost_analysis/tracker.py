...
class CostTracker:
    ...
    def record_llm_usage(
        ...
        input_event = CostEvent(
            ...
            amount_usd=cost * (input_tokens / (input_tokens + output_tokens)) if (input_tokens + output_tokens) > 0 else 0,
        ...
        output_event = CostEvent(
            ...
            amount_usd=cost * (output_tokens / (input_tokens + output_tokens)) if (input_tokens + output_tokens) > 0 else 0,
            agent_type=agent_type,
            point_id=point_id,
            tour_id=tour_id,
            metadata={
        ...
        with self._lock:
            ...
            self._category_totals[CostCategory.LLM_INPUT_TOKENS] += input_event.amount_usd
            self._category_totals[CostCategory.LLM_OUTPUT_TOKENS] += output_event.amount_usd
    ...
    def get_total_cost(self) -> float:
        ...
        with self._lock:
            return sum(
                ...
                if k not in {CostCategory.CACHE_HIT_SAVINGS, CostCategory.CIRCUIT_BREAKER_SAVINGS}
            )
    ...
    def get_total_savings(self) -> float:
        ...
        with self._lock:
            return abs(sum(
                v for k, v in self._category_totals.items()
                if k in {CostCategory.CACHE_HIT_SAVINGS, CostCategory.CIRCUIT_BREAKER_SAVINGS}
            ))
    ...
    def persist(self) -> None:
        ...
        with self._lock:
            data = {
                ...
                "category_totals": {k.value: v for k, v in self._category_totals.items()},
    ...
    def complete(self) -> TourCostSummary:
        ...
        for event in events:
            if event.tour_id == self.tour_id:
                if event.category in {CostCategory.LLM_INPUT_TOKENS, CostCategory.LLM_OUTPUT_TOKENS}:
                ...
                elif event.category in {CostCategory.API_GOOGLE_MAPS, CostCategory.API_YOUTUBE, CostCategory.API_WEB_SEARCH}:
