...
class GoogleMapsClient:
        ...
        for leg in route_data.get("legs", []):
            ...
            for step in leg.get("steps", []):
                ...
                address = self._get_address_from_location(
                    end_location.get("lat"), end_location.get("lng")
                )
                ...
                points.append(
                    RoutePoint(
                        index=point_index,
                        address=address
                        or f"Point at {end_location.get('lat'):.4f}, {end_location.get('lng'):.4f}",
    ...
    def _get_address_from_location(self, lat: float, lng: float) -> str | None:
        """
    ...
    def _extract_location_name(self, address: str) -> str | None:
        """
    ...
    def get_place_details(self, place_name: str) -> dict | None:
        """
        ...
        try:
            ...
            places_result = self.client.places(
                query=place_name, language=settings.language
            )
...
def get_mock_route(
    origin: str = "Tel Aviv, Israel", destination: str = "Jerusalem, Israel"
) -> Route:
    """
