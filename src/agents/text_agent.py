    ...
    def _search_content(self, point: RoutePoint) -> Optional[ContentResult]:
        ...
        if content:
            return ContentResult(
                ...
                title=content.get('title', f'About {point.location_name or point.address}'),
...
Mix Hebrew and English queries for better coverage."""
        ...
        try:
            ...
            return queries[:3] if queries else [f"{location} history", f"{location} facts"]
        except Exception:
            return [
                f"{location} history",
    ...
    def _synthesize_content(self, results: List[Dict], point: RoutePoint) -> Optional[Dict]:
...
SCORE: [Relevance score 0-10]"""
        ...
        try:
            ...
            return {
                'title': title_match.group(1).strip() if title_match else f"About {location}",
                'fact_type': type_match.group(1).strip() if type_match else 'general',
                'story': story_match.group(1).strip() if story_match else response[:200],
                ...
                'is_historical': 'historical' in (type_match.group(1) if type_match else '')
            }
