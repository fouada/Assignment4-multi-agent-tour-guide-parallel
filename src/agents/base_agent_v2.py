...
# Core imports
from src.core.plugins.base import BasePlugin, PluginMetadata, PluginState
from src.core.plugins.events import Event, EventBus, publish
from src.core.plugins.hooks import HookRegistry, HookType, hookable
...
from src.core.di import inject, get_container
...
class EnhancedBaseAgent(ABC, Generic[TConfig]):
    ...
    config_class: ClassVar[Optional[Type[BaseModel]]] = AgentConfig
    ...
    def __init__(self, config: Optional[TConfig] = None):
        ...
        # Tracking
        self._current_point_id: Optional[str] = None
    ...
    def name(self) -> str:
        ...
        return getattr(self.__class__, 'metadata', 
                      AgentMetadata(name=self.__class__.__name__)).name
    
    @property
    def version(self) -> str:
        """Agent version from metadata."""
        return getattr(self.__class__, 'metadata',
                      AgentMetadata(name=self.__class__.__name__)).version
    ...
    def execute(self, point: RoutePoint) -> Optional[ContentResult]:
        ...
        with tracer.span(f"{self.name}.execute") as span:
            ...
            try:
                ...
                publish(AgentExecutionCompleted(
                    source=self.name,
                    agent_name=self.name,
                    agent_type=self.__class__.__name__,
                    point_id=point.id,
    ...
    def add_post_execute_hook(
        self,
        hook: Callable[[RoutePoint, Optional[ContentResult]], None],
    ) -> None:
