...
class SystemMonitorPanel:
    ...
    def create_agent_status_cards(metrics: dict) -> go.Figure:
        ...
        for i, (_agent, data) in enumerate(agents.items()):
            status_color = (
                COLORS["healthy"] if data["status"] == "healthy" else COLORS["degraded"]
            )
            ...
            fig.add_trace(
                go.Indicator(
                    ...
                    number={
                        "suffix": "%",
                        "font": {"size": 28, "family": FONTS["mono"]},
                    },
                    delta={
                        "reference": 95,
                        "increasing": {"color": COLORS["complete"]},
                    },
                    gauge={
    ...
    def create_queue_depth_chart(depth_history: list[int]) -> go.Figure:
        ...
        fig.add_hline(
            y=10,
            line_dash="dash",
            line_color=COLORS["soft_degraded"],
...
class SensitivityPanel:
    ...
    def create_parameter_impact_chart(data: pd.DataFrame, param_name: str) -> go.Figure:
        ...
        layout = create_base_layout(
            f"🔬 Sensitivity Analysis: {param_name.replace('_', ' ').title()}", 650
        )
...
class ParetoFrontierPanel:
    ...
    def create_3d_pareto_surface(data: pd.DataFrame) -> go.Figure:
        ...
        fig = go.Figure(
            data=[
                go.Surface(
                    ...
                    colorscale="Viridis",
                    colorbar={
                        "title": {"text": "Quality", "font": {"family": FONTS["mono"]}}
                    },
                    opacity=0.9,
                    contours={
                        "z": {
        ...
        fig.update_layout(
            ...
            scene={
                "xaxis": {
                    "title": {"text": "Hard Timeout (s)"},
                    "backgroundcolor": "rgba(22,33,62,0.8)",
                },
                "yaxis": {
                    "title": {"text": "Soft Timeout (s)"},
                    "backgroundcolor": "rgba(22,33,62,0.8)",
                },
                "zaxis": {
    ...
    def create_pareto_scatter(data: pd.DataFrame) -> go.Figure:
        ...
        for i, row in data.iterrows():
            ...
            for j, other in data.iterrows():
                if i != j:
                    ...
                    if (
                        other["latency_mean"] <= row["latency_mean"]
                        and other["quality_mean"] >= row["quality_mean"]
                    ):
                        if (
                            other["latency_mean"] < row["latency_mean"]
                            or other["quality_mean"] > row["quality_mean"]
                        ):
                            is_pareto = False
                            break
        ...
        if len(pareto_data) > 0:
            ...
            fig.add_annotation(
                x=best_quality["latency_mean"],
                y=best_quality["quality_mean"],
                text="High Quality",
                showarrow=True,
                arrowhead=2,
            ...
            fig.add_annotation(
                x=best_latency["latency_mean"],
                y=best_latency["quality_mean"],
                text="Low Latency",
                showarrow=True,
                arrowhead=2,
...
class StatisticalComparisonPanel:
    ...
    def create_distribution_comparison(
        ...
        status_colors = [
            COLORS["complete"],
            COLORS["soft_degraded"],
            COLORS["hard_degraded"],
            COLORS["failed"],
        ]

        for status, color in zip(statuses, status_colors, strict=False):
            fig.add_trace(
                go.Bar(
        ...
        for df, name, color in [
            (df_a, config_names[0], COLORS["comparison"][0]),
    ...
    def create_effect_size_chart(comparison: dict) -> go.Figure:
        ...
        effect_sizes = [
            comparison["latency"]["cohens_d"],
            comparison["quality"]["cohens_d"],
        ]
        ...
        # Effect size bars
        colors = [
            COLORS["comparison"][1] if d > 0 else COLORS["comparison"][0]
    ...
    def create_significance_summary(comparison: dict) -> go.Figure:
        ...
        for i, (metric, p) in enumerate(zip(metrics, p_values, strict=False)):
            sig_level = (
                "p < 0.001 ✓✓✓"
                if p < 0.001
                else "p < 0.01 ✓✓"
            ...
            fig.add_trace(
                go.Indicator(
                    ...
                    value=p,
                    number={
                        "font": {"size": 32, "family": FONTS["mono"]},
                        "valueformat": ".2e",
                    },
                    delta={
                        "reference": 0.05,
                        "decreasing": {"color": COLORS["complete"]},
                    },
                    title={
                        "text": f'{metric}<br><span style="font-size:0.8em;color:{color}">{sig_level}</span>'
                    },
...
class MonteCarloPanel:
    ...
    def create_simulation_results(df: pd.DataFrame) -> go.Figure:
        ...
        fig.update_layout(
            **create_base_layout(
                f"🎲 Monte Carlo Simulation Results (N={len(df):,})", 650
            ),
            showlegend=False,
        )
        ...
        if len(n_values) > 1:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=[v + s for v, s in zip(metric_values, stds, strict=False)]
                    + [
                        v - s
                        for v, s in zip(metric_values[::-1], stds[::-1], strict=False)
                    ],
                    fill="toself",
                    fillcolor="rgba(233, 69, 96, 0.2)",
        ...
        for i, col in enumerate(success_cols):
            ...
            fig.add_trace(
                go.Indicator(
                    ...
                    number={
                        "suffix": "%",
                        "font": {"family": FONTS["mono"], "size": 24},
                    },
