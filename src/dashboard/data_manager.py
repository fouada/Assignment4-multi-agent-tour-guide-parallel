...
class SmartQueueSimulator:
    ...
    DEFAULT_AGENTS = [
        AgentConfig("video", mu=1.0, sigma=0.5, shift=0.5, reliability=0.92, 
                   quality_mean=7.5, quality_std=1.5),
        AgentConfig("music", mu=0.8, sigma=0.4, shift=0.3, reliability=0.95, 
                   quality_mean=7.0, quality_std=1.2),
        AgentConfig("text", mu=0.6, sigma=0.3, shift=0.2, reliability=0.98, 
                   quality_mean=6.5, quality_std=1.0),
    ]
    ...
    def simulate_single(self) -> SimulationResult:
        ...
        for agent in self.agents:
            ...
            quality = np.clip(np.random.normal(agent.quality_mean, agent.quality_std), 0, 10) if success else 0.0
        ...
        successes = [(n, t, agent_quality[n]) for n, t in sorted_agents if agent_success[n]]
        
        # Apply tiered timeout logic
        if len(successes) == 0:
            return SimulationResult(QueueStatus.FAILED, cfg.hard_timeout, 0, 0.0, 
                                   agent_times, agent_success)
        ...
        if len(successes) >= cfg.expected_agents:
            ...
            if max_time <= cfg.hard_timeout:
                ...
                status = QueueStatus.SOFT_DEGRADED if num_results >= cfg.min_for_soft else QueueStatus.HARD_DEGRADED
        ...
        return SimulationResult(status, latency, num_results, quality, agent_times, agent_success)
...
class DashboardDataManager:
    ...
    def get_baseline_simulation(self, n_sims: int = 5000, 
    ...
    def compare_configurations(
        ...
        d_qual = (np.mean(qual_a) - np.mean(qual_b)) / pooled_qual if pooled_qual > 0 else 0
        ...
        return {
            ...
            'df_b': df_b,
            'latency': {
                'mean_a': np.mean(lat_a), 'mean_b': np.mean(lat_b),
                'std_a': np.std(lat_a), 'std_b': np.std(lat_b),
                'p_value': p_lat, 'cohens_d': d_lat,
            },
            'quality': {
                'mean_a': np.mean(qual_a), 'mean_b': np.mean(qual_b),
                'std_a': np.std(qual_a), 'std_b': np.std(qual_b),
                'p_value': p_qual, 'cohens_d': d_qual,
            },
    ...
    def get_real_time_metrics(self) -> dict:
        ...
        return {
            ...
            'agents': {
                'video': {
                    'status': np.random.choice(['healthy', 'healthy', 'degraded'], p=[0.8, 0.15, 0.05]),
                    'avg_response_time': np.random.uniform(2, 5),
                    'success_rate': np.random.uniform(0.85, 0.98),
                },
                'music': {
                    'status': np.random.choice(['healthy', 'healthy', 'degraded'], p=[0.85, 0.12, 0.03]),
    ...
    def export_results(self, output_path: Path) -> None:
        ...
        export_data = {
            'timestamp': datetime.now().isoformat(),
            'baseline_stats': self._baseline_data.describe().to_dict() if self._baseline_data is not None else None,
            'sensitivity_data': {k: v.to_dict() for k, v in self._sensitivity_data.items()},
        }
