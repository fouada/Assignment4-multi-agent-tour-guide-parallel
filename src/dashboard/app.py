...
def create_app() -> Dash:
    ...
    app.layout = html.Div([
        ...
        html.Div([
            # Header
            html.Div([
                html.H1("🗺️ Multi-Agent Tour Guide", className="dashboard-title"),
                html.P("MIT-Level Research Dashboard | Parallel Processing Analysis", 
            ...
            dcc.Tabs([
                ...
                # ============================================================
                dcc.Tab(label='📊 System Monitor', className='custom-tab', 
                        selected_className='custom-tab--selected', children=[
                    html.Div([
                        # Real-time metrics row
                        html.Div([
                            html.Div([
                                html.Div(id='metric-active-tours', className='metric-value'),
                                html.Div('Active Tours', className='metric-label'),
                            ], className='metric-card'),
                            html.Div([
                                html.Div(id='metric-queue-depth', className='metric-value'),
                                html.Div('Queue Depth', className='metric-label'),
                            ], className='metric-card'),
                            html.Div([
                                html.Div(id='metric-throughput', className='metric-value'),
                                html.Div('Throughput (req/s)', className='metric-label'),
                            ], className='metric-card'),
                            html.Div([
                                html.Div(id='metric-error-rate', className='metric-value'),
                                html.Div('Error Rate', className='metric-label'),
                            ], className='metric-card'),
                        ], className='grid-4'),
                        
                        # Agent status cards
                        html.Div([
                            dcc.Graph(id='agent-status-graph', config={'displayModeBar': False}),
                        ], className='card'),
                        ...
                        html.Div([
                            ...
                            html.Div([
                                dcc.Graph(id='queue-depth-chart', config={'displayModeBar': False}),
                            ], className='card'),
                        ], className='grid-2'),
                    ], style={'padding': '20px'}),
                ]),
                ...
                # ============================================================
                dcc.Tab(label='🔬 Sensitivity Analysis', className='custom-tab',
                        selected_className='custom-tab--selected', children=[
                    html.Div([
                        # Control panel
                        html.Div([
                            html.H4('🎛️ Analysis Controls', className='card-title'),
                            html.Div([
                                html.Div([
                                    html.Label('Parameter to Analyze', className='slider-label'),
                                    dcc.Dropdown(
                                        id='sensitivity-param-dropdown',
                                        options=[
                                            {'label': 'Soft Timeout', 'value': 'soft_timeout'},
                                            {'label': 'Hard Timeout', 'value': 'hard_timeout'},
                                        ],
                                        value='soft_timeout',
                                ...
                                html.Div([
                                    html.Label('Parameter Range', className='slider-label'),
                                    dcc.RangeSlider(
                                        id='sensitivity-range-slider',
                                        min=1, max=60, step=1,
                                        value=[5, 30],
                                        marks={i: f'{i}s' for i in range(0, 65, 10)},
                                        tooltip={'placement': 'bottom', 'always_visible': True},
                                    ),
                                ], style={'flex': 2, 'marginRight': '20px'}),
                                
                                html.Div([
                                    html.Label('Simulations per Point', className='slider-label'),
                                    dcc.Input(
                            ...
                            ], style={'display': 'flex', 'alignItems': 'flex-end', 'flexWrap': 'wrap', 'gap': '15px'}),
                        ], className='control-panel'),
                        
                        # Progress indicator
                        html.Div(id='sensitivity-progress', style={'marginBottom': '20px'}),
                        
                        # Results
                        html.Div([
                            dcc.Loading(
                                dcc.Graph(id='sensitivity-results-graph'),
                                type='circle',
                ...
                # ============================================================
                dcc.Tab(label='🎯 Pareto Explorer', className='custom-tab',
                        selected_className='custom-tab--selected', children=[
                    html.Div([
                        # Controls
                        html.Div([
                            html.H4('🗻 Pareto Frontier Controls', className='card-title'),
                            html.Div([
                                html.Div([
                                    html.Label('Soft Timeout Range', className='slider-label'),
                                    dcc.RangeSlider(
                                        id='pareto-soft-range',
                                        min=1, max=40, step=1,
                                        value=[5, 30],
                                        marks={i: f'{i}s' for i in range(0, 45, 10)},
                                    ),
                                ], style={'flex': 1, 'marginRight': '30px'}),
                                
                                html.Div([
                                    html.Label('Hard Timeout Range', className='slider-label'),
                                    dcc.RangeSlider(
                                        id='pareto-hard-range',
                                        min=5, max=80, step=1,
                                        value=[10, 60],
                                        marks={i: f'{i}s' for i in range(0, 85, 20)},
                                    ),
                                ], style={'flex': 1, 'marginRight': '30px'}),
                        ...
                        html.Div([
                            html.Div([
                                dcc.Loading(
                                    dcc.Graph(id='pareto-3d-surface'),
                                    type='circle',
                                    color=COLORS['highlight'],
                                ),
                            ], className='card'),
                ...
                # ============================================================
                dcc.Tab(label='📐 A/B Testing', className='custom-tab',
                        selected_className='custom-tab--selected', children=[
                    html.Div([
                        # Configuration panels
                        html.Div([
                            html.H4('⚖️ Configuration Comparison', className='card-title'),
                            html.Div([
                                # Config A
                                html.Div([
                                    html.H5('Configuration A', style={'color': COLORS['comparison'][0]}),
                                    html.Label('Soft Timeout (s)', className='slider-label'),
                                    dcc.Input(id='config-a-soft', type='number', value=15, min=1, max=60),
                                    html.Label('Hard Timeout (s)', className='slider-label', style={'marginTop': '10px'}),
                                    dcc.Input(id='config-a-hard', type='number', value=30, min=5, max=120),
                                ...
                                # Config B
                                html.Div([
                                    html.H5('Configuration B', style={'color': COLORS['comparison'][1]}),
                                    html.Label('Soft Timeout (s)', className='slider-label'),
                                    dcc.Input(id='config-b-soft', type='number', value=8, min=1, max=60),
                                    html.Label('Hard Timeout (s)', className='slider-label', style={'marginTop': '10px'}),
                                    dcc.Input(id='config-b-hard', type='number', value=15, min=5, max=120),
                                ], className='card', style={'flex': 1}),
                            ], style={'display': 'flex', 'gap': '20px', 'alignItems': 'stretch'}),
                            
                            html.Div([
                                html.Label('Simulations per Config', className='slider-label'),
                                dcc.Input(id='comparison-n-sims', type='number', value=5000, min=1000, max=20000, step=1000),
                                html.Button('⚡ Compare', id='run-comparison-btn', className='btn-primary', 
                                           style={'marginLeft': '20px'}),
                            ], style={'marginTop': '20px', 'display': 'flex', 'alignItems': 'flex-end'}),
                        ], className='control-panel'),
                        
                        # Results
                        html.Div([
                            dcc.Loading(
                                dcc.Graph(id='comparison-distributions'),
                                type='circle',
                                color=COLORS['highlight'],
                            ),
                        ], className='card'),
                        ...
                        html.Div([
                            html.Div([
                                dcc.Graph(id='effect-size-chart'),
                            ], className='card'),
                            html.Div([
                                dcc.Graph(id='significance-summary'),
                            ], className='card'),
                        ], className='grid-2'),
                    ], style={'padding': '20px'}),
                ]),
                ...
                # Tab 5: Monte Carlo
                # ============================================================
                dcc.Tab(label='🎲 Monte Carlo', className='custom-tab',
                        selected_className='custom-tab--selected', children=[
                    html.Div([
                        # Controls
                        html.Div([
                            html.H4('🎲 Monte Carlo Simulation', className='card-title'),
                            html.Div([
                                html.Div([
                                    html.Label('Soft Timeout (s)', className='slider-label'),
                                    dcc.Slider(
                                        id='mc-soft-timeout',
                                        min=1, max=40, step=1, value=15,
                                        marks={i: f'{i}' for i in range(0, 45, 10)},
                                    ),
                                ], style={'flex': 1, 'marginRight': '30px'}),
                                
                                html.Div([
                                    html.Label('Hard Timeout (s)', className='slider-label'),
                                    dcc.Slider(
                                        id='mc-hard-timeout',
                                        min=5, max=80, step=1, value=30,
                                        marks={i: f'{i}' for i in range(0, 85, 20)},
                                    ),
                                ], style={'flex': 1, 'marginRight': '30px'}),
                                
                                html.Div([
                                    html.Label('Number of Simulations', className='slider-label'),
                                    dcc.Input(id='mc-n-sims', type='number', value=5000, min=100, max=50000, step=1000),
                                ], style={'marginRight': '20px'}),
                                
                                html.Button('🎲 Run Simulation', id='run-mc-btn', className='btn-primary'),
                            ], style={'display': 'flex', 'alignItems': 'flex-end', 'flexWrap': 'wrap', 'gap': '20px'}),
                        ], className='control-panel'),
                        
                        # Summary stats
                        html.Div(id='mc-summary-stats', className='grid-4', style={'marginBottom': '20px'}),
                        
                        # Main results
                        html.Div([
                            dcc.Loading(
                                dcc.Graph(id='mc-results-graph'),
                                type='circle',
                                color=COLORS['highlight'],
                            ),
                        ], className='card'),
                        ...
                        html.Div([
                            html.Div([
                                dcc.Graph(id='mc-agent-response'),
                            ], className='card'),
                            html.Div([
                                dcc.Graph(id='mc-agent-reliability'),
                            ], className='card'),
                        ], className='grid-2'),
                    ], style={'padding': '20px'}),
                ]),
    ...
    def update_system_monitor(n):
        ...
        return active_tours, queue_depth, throughput, error_rate, agent_fig, throughput_fig, queue_fig
    ...
    def run_sensitivity_analysis(n_clicks, param, range_val, n_sims):
        ...
        sensitivities = {
            'Latency Impact': (results_df['latency_mean'].iloc[-1] - results_df['latency_mean'].iloc[0]) / results_df['latency_mean'].mean(),
            'Quality Impact': (results_df['quality_mean'].iloc[-1] - results_df['quality_mean'].iloc[0]) / results_df['quality_mean'].mean(),
            'Success Rate': (results_df['success_rate'].iloc[-1] - results_df['success_rate'].iloc[0]),
            'Complete Rate': (results_df['complete_rate'].iloc[-1] - results_df['complete_rate'].iloc[0]),
        }
    ...
    @app.callback(
        ...
        [State('pareto-soft-range', 'value'),
         State('pareto-hard-range', 'value')],
        prevent_initial_call=True
    )
    ...
    def run_comparison(n_clicks, a_soft, a_hard, b_soft, b_hard, n_sims):
        ...
        dist_fig = StatisticalComparisonPanel.create_distribution_comparison(
            comparison['df_a'], comparison['df_b'],
    ...
    def run_monte_carlo(n_clicks, soft_timeout, hard_timeout, n_sims):
        ...
        summary_cards = [
            ...
            html.Div([
                html.Div(f"{(df['status'] == 'complete').mean():.1%}", className='metric-value'),
                html.Div('Complete Rate', className='metric-label'),
            ], className='metric-card'),
            html.Div([
                html.Div(f"{(df['status'] != 'failed').mean():.1%}", className='metric-value'),
                html.Div('Success Rate', className='metric-label'),
            ], className='metric-card'),
        ]
        
        return results_fig, response_fig, reliability_fig, summary_cards, df.head(100).to_dict()
