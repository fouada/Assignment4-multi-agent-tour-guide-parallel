...
class PluginRegistry:
    ...
    def register_plugin(
        ...
        *,
        ...
        with cls._lock:
            ...
            logger.info(
                f"Registered plugin: {name} v{cls._metadata[name].version}"
            )
    ...
    @classmethod
    def _load_plugin_from_manifest(cls, manifest_path: Path) -> Optional[str]:
        ...
        if not name:
            raise PluginRegistrationError(f"Plugin manifest missing 'name': {manifest_path}")
        ...
        if not module_file.exists():
            raise PluginRegistrationError(
                f"Plugin module not found in {plugin_dir}"
            )
        ...
        if not plugin_class:
            raise PluginRegistrationError(
                f"No plugin class found in {module_file}"
            )
    ...
    @classmethod
    def add_search_path(cls, path: Union[str, Path]) -> None:
